...
export async function createProject(data: CreateProjectData): Promise<Project> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.detail || `Failed to create project: ${response.statusText}`,
      );
    }
...
export async function getProjects(): Promise<Project[]> {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || `Failed to fetch projects: ${response.statusText}`,
      );
    }
...
export async function deleteProject(projectId: string): Promise<void> {
  try {
    ...
    if (planFiles.length > 0) {
      await createClient().storage.from("plans").remove(planFiles);
    }
    ...
    if (photoFiles.length > 0) {
      await createClient().storage.from("photos").remove(photoFiles);
    }
  } catch (error) {
...
export async function savePhotoPlacement(
  ...
  try {
    ...
    const { data: existingPlacement } = await createClient()
 ...
 */
export async function getProjectPlacements(
  projectId: string,
): Promise<PhotoPlacement[]> {
  try {
...
export async function setPlanActive(
  ...
  try {
    ...
    const { data: currentActivePlan } = await createClient()
      .from("plans")
      .select("id")
      .eq("project_id", projectId)
      .eq("is_active", true)
    ...
    const { error: resetError } = await createClient()
      .from("plans")
    ...
    const { error: setError } = await createClient()
      .from("plans")
...
export async function uploadPlan(projectId: string, file: File): Promise<Plan> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.detail || `Failed to upload plan: ${response.statusText}`,
      );
    }
...
export async function deletePlan(planId: string): Promise<void> {
  try {
    ...
    const { data: plan, error: fetchError } = await createClient()
      .from("plans")
      .select("file_url")
      .eq("id", planId)
      .single();
    ...
    if (plan?.file_url) {
      ...
      if (urlParts[1]) {
        await createClient().storage.from("plans").remove([urlParts[1]]);
      }
    }
  } catch (error) {
...
export async function uploadPhoto(
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = createClient().storage.from("photos").getPublicUrl(fileName);

    // Extract EXIF data
    const exifData = await extractExifData(file);

    // Create photo record in database
    const { data: photoData, error: dbError } = await createClient()
      .from("photos")
      .insert({
        id: crypto.randomUUID(),
    ...
    if (dbError) {
      ...
      await createClient().storage.from("photos").remove([fileName]);
    ...
    if (autoPlace && photoData) {
      try {
        const { x, y } = generateRandomPlacement(
          autoPlace.planWidth,
          autoPlace.planHeight,
        );
...
export async function deletePhotos(photoIds: string[]): Promise<void> {
  try {
    ...
    const { data: photos, error: fetchError } = await createClient()
      .from("photos")
      .select("file_url")
      .in("id", photoIds);
...
export async function exportProject(
  projectId: string,
): Promise<{ project_id: string; export_url: string }> {
  try {
    const headers = await getAuthHeaders();
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.message || `Failed to export project: ${response.statusText}`,
      );
    }
...
export async function createReport(projectId: string): Promise<Report> {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || `Failed to create report: ${response.statusText}`,
      );
    }
...
export async function getProjectReports(projectId: string): Promise<Report[]> {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || `Failed to fetch reports: ${response.statusText}`,
      );
    }
