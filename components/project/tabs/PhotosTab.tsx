...
export default function PhotosTab({ projectId }: PhotosTabProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 2. Fetch Placements
        const { data: plansData } = await createClient()
          .from("plans")
          .select("*")
        ...
        if (planIds.length > 0) {
          const { data: placements } = await createClient()
            .from("photo_placements")
            .select("photo_id")
            .in("plan_id", planIds);

          const placedIds = new Set(
            placements?.map((p: any) => p.photo_id) || [],
          );
  ...
  const handleAutoPlace = async (photoId: string) => {
    try {
      ...
      if (!activePlan) {
        setToast({
          message: "No floor plan found to place photo on.",
          type: "error",
        });
        return;
      }
  ...
  const handleBulkAutoPlace = async () => {
    ...
    if (!activePlan) {
      setToast({
        message: "No floor plan found to place photos on.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      setToast({
        message: `Successfully auto-placed ${ids.length} photos!`,
        type: "success",
      });
  ...
  const handleFileSelect = async (files: FileList | null) => {
    ...
    if (invalidFiles.length > 0) {
      setError(
        "Some files were skipped. Only valid image files (JPG, PNG, WebP) are allowed.",
      );
    }
  ...
  const confirmDelete = async () => {
    ...
    if (photos.length - selectedPhotoIds.size < 1) {
      setToast({
        message: "You must keep at least one photo in the project.",
        type: "error",
      });
    ...
    try {
      ...
      setToast({
        message: `${idsToDelete.length} photo(s) deleted successfully`,
        type: "success",
      });
      // Reset selection
      setSelectedPhotoIds(new Set());
    ...
    return (
      <div className="border-2 border-dashed border-gray-200 rounded-xl p-12 text-center hover:bg-gray-50 transition-colors">
        ...
        <p className="text-gray-500 mb-6 max-w-md mx-auto">
          Upload geotagged photos to place them on your floor plan and create
          documentation.
        </p>
  ...
  return (
    <div className="space-y-6">
      ...
      {toast && (
        <div
          className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-lg shadow-lg text-sm font-medium animate-in fade-in slide-in-from-top-4 duration-300 ${
            toast.type === "error"
