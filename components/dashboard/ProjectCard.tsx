...
import { Download, MapPin, FileStack, Clock, Loader2 } from "lucide-react";
...
export default function ProjectCard({ project, onDelete }: ProjectCardProps) {
    ...
    return (
        <div className="group relative bg-white rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col h-full">
            {/* Project Thumbnail & Image */}
            <Link href={`/projects/${project.id}`} className="block relative aspect-video overflow-hidden bg-gray-100">
                <Image
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium backdrop-blur-md shadow-sm ${project.status === 'completed' ? 'bg-emerald-500/90 text-white' :
                        project.status === 'processing' ? 'bg-blue-500/90 text-white' :
            ...
            <div className="p-5 flex-grow flex flex-col">
                ...
                <div className="flex items-center gap-2 relative">
                    ...
                    {toast && (
                        <div className={`absolute bottom-full left-0 right-0 mb-2 px-3 py-2 rounded-lg text-xs font-medium text-white shadow-lg text-center z-50 ${toast.type === 'error' ? 'bg-red-600' : 'bg-emerald-600'
                            }`}>
                            {toast.message}
                        </div>
                    )}
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                    >
                        {isExporting ? (
                            <>
